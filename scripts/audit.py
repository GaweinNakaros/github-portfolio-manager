...
class GitHubAuditor:
    ...
    def show_recommendations(self, repos):
        ...
        if prof_count < 3:
            msg = ("• Considera convertir repos académicos " +
        ...
        if private_repos:
            msg = (f"• Tienes {len(private_repos)} repos privados - " +
                   "evalúa hacerlos públicos")
            recommendations.append(msg)

        no_description = [r for r in repos if not r['description']]
        if no_description:
            msg = (f"• {len(no_description)} repos sin descripción - " +
...
def main():
    parser = argparse.ArgumentParser(
        description='Auditoría profesional de repositorios GitHub'
    )
    parser.add_argument(
        '--user', '-u', default='GaweinNakaros',
        help='Usuario de GitHub a auditar'
    )
    parser.add_argument(
        '--output', '-o', default='data/audits',
